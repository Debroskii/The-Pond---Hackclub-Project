...
class draw:
    ...
    def multi_radii_line(surface, color, start: pygame.Vector2, end: pygame.Vector2, start_radius: int, end_radius: int):
        ...
        pygame.gfxdraw.aacircle(surface, int(start.x), int(start.y), int(start_radius), color)
        pygame.gfxdraw.aacircle(surface, int(end.x), int(end.y), int(end_radius), color)    
        pygame.gfxdraw.filled_polygon(surface, points, color)
        pygame.gfxdraw.filled_circle(surface, int(start.x), int(start.y), int(start_radius), color)
        pygame.gfxdraw.filled_circle(surface, int(end.x), int(end.y), int(end_radius), color)     
    ...
    def fract_circle(surface, color, position, radius, angle, span, resolution = 1):
        ...
        poly_surf = pygame.Surface(pygame.display.get_window_size(), pygame.SRCALPHA)
        pygame.draw.polygon(poly_surf, (color[0], color[1], color[2], 50), points)
