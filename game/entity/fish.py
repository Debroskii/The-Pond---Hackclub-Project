...
class Fish:
    def __init__(self, color_config, scale: float):
        ...
        self.scale = scale
        self.kine_chain = UIKinematicsChain([20 * scale, 20 * scale, 20 * scale, 20 * scale, 5 * scale, 7.5 * scale], math.pi / 4)
    ...
    def update(self, timestamp, dt):
        speed_modifier = 1
        if self.path_entity.mode != PathMode.WANDER: 
            speed_modifier = max(0.1, min((self.path_entity.target - self.path_entity.pos).magnitude() * 0.01, 2))
        self.path_entity.loop(timestamp, game_manager.GameManager.fish.index(self), speed_modifier, dt)
